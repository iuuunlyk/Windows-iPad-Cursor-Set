...
觉得指针太小，可以用 Windows 11 的全局大小滑块调大，本套件的 11 档内嵌尺寸在调大后依然保持清晰。注意**操作顺序必须是下面两步**：

1. **设置 → 辅助功能 → 鼠标指针和触控**，把「大小」滑块拖到目标档位。
   拖动后指针会暂时变回系统默认样式——这是 Windows 的正常行为（辅助功能页会接管当前指针样式），不是套件失效，进入第 2 步恢复即可。
2. **控制面板 → 鼠标 → 「指针」选项卡**，在「方案」下拉框重新选择 “iPad-Cursor-Set” → 「应用」→「确定」。
   重选后恢复套件样式，且**尺寸保留第 1 步调好的大小**。

**建议的最大尺寸档位**（「大小」滑块的数字档位，1–15）：
...
原理：Windows 按「需求尺寸 = 滑块对应基准尺寸 × 显示缩放」请求光标帧（滑块 1–15 对应基准尺寸 32–256px，步进 16px，即档位 N 的基准 = 16 × (N+1)；例：150% 缩放下档位 4 → 80 × 1.5 = 120px，仍在 128px 以内；档位 5 → 96 × 1.5 = 144px，超限）。本套件最大内嵌帧为 128px，需求超过该值时 Windows 只能放大位图渲染，边缘出现锯齿。上表按「需求尺寸 ≤ 128px」给出各缩放档位的安全上限（已向下取整留余量）。调大滑块本身不会降低清晰度，**只要不超上限**，11 档内嵌尺寸始终有匹配帧。
...
本套件、预览图与文档由作者提出需求、确认方案并逐项验收，AI 在作者指导下执行产出；几何形状源自对参考图像的路径解析与像素量测。
